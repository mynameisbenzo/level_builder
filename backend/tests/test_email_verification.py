...
def test_signing_up_with_email_issues_a_verification_token():
    ...
    with app.app_context():
        ...
        token = EmailVerificationToken.query.filter_by(user_id=user_id).first()
...
def test_signing_up_with_only_twitch_issues_no_token():
    ...
    with app.app_context():
        ...
        assert EmailVerificationToken.query.filter_by(user_id=user_id).first() is None
...
def test_dev_verification_token_included_in_response_when_debug():
    ...
    with app.app_context():
        ...
        token = EmailVerificationToken.query.filter_by(user_id=body["id"]).first()
...
def test_verify_email_with_valid_token_marks_user_verified():
    ...
    with app.app_context():
        ...
        refetched_user = db.session.get(User, user_id)
